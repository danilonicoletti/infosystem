...
$columns = array('codigo', 'descricao', 'parcela_1', 'parcela_2', 'parcela_3', 'parcela_4', 'parcela_5', 'parcela_6', 'gerar_contas_receber', 'id_empresa');
...
/* Begin::Adicionar */
if ($type == 'forma_de_pagamento_add') {

    try {
        $sql = 'INSERT INTO forma_de_pagamento (' . $column_insert . ') VALUES(' . $param_insert . ')';
        ...
        $escreve = fwrite($fp, $number_error . "|forma_de_pagamento_add|" . debugPDO($sql, $parameters) . "\n\n");
        fclose($fp);
...
/* Begin::Editar */
if ($type == 'forma_de_pagamento_edit') {
    $cliente_error = 0;

    try {
        $sql = 'UPDATE forma_de_pagamento SET ' . $param_update . ' WHERE id_pagamento = ' . $_POST['id'];
        $stmt = $pdo->prepare($sql);
        $parameters = array(
            ':codigo' => filter_input(INPUT_POST, "codigo", FILTER_DEFAULT),
            ':descricao' => filter_input(INPUT_POST, "descricao", FILTER_DEFAULT),
        ...
        $fp = fopen("../../report/error_log_" . date('dmY') . ".txt", "a");
        $escreve = fwrite($fp, $number_error . "|forma_de_pagamento_edit|" . debugPDO($sql, $parameters) . "\n\n");
        fclose($fp);
...
/* Begin::Remover */
if ($type == 'forma_de_pagamento_remove') {

    try {
        $stmt = $pdo->prepare('DELETE FROM forma_de_pagamento WHERE id_pagamento = :id');
        ...
        $fp = fopen("../../report/error_log_" . date('dmY') . ".txt", "a");
        $escreve = fwrite($fp, $number_error . "|forma_de_pagamento_delete|DELETE FROM forma_de_pagamento WHERE id_pagamento = " . $_POST['id'] . "\n\n");
        fclose($fp);
